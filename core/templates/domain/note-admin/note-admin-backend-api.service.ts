...
export interface NoteAdminPageDataBackendDict {
...
export interface NoteAdminPageData {
...
export class NoteAdminBackendApiService {
  constructor(private http: HttpClient) {}

  async getDataAsync(): Promise<NoteAdminPageData> {
    return new Promise((resolve, reject) => {
      this.http
        .get<NoteAdminPageDataBackendDict>('/noteadminhandler')
        .toPromise()
        .then(
          response => {
            resolve({
  ...
  async updateUserRoleAsync(newRole: string, username: string): Promise<void> {
    ...
    return this._postRequestAsync('/noteadminrolehandler', payload);
  }

  async removeNoteEditorAsync(username: string): Promise<Object> {
    return this.http
      .put('/noteadminrolehandler', {
        username: username,
      })
      .toPromise();
  }
...
angular
  ...
  .factory(
    ...
    downgradeInjectable(NoteAdminBackendApiService)
  );
